...
//    NSString *userDataDir = [[[[NSBundle mainBundle] infoDictionary] objectForKey:kXIMEUserDataDirectoryKey] stringByStandardizingPath];
    ...
    userDataDir = [NSString stringWithFormat:@"%@/XIME", Dir];
//    userDataDir = [NSString rimeResource];
    ...
    vXIMETraits.shared_data_dir = [[NSString stringWithFormat:@"%@/SharedSupport", Dir] UTF8String];
...
+ (NSArray *)getCandidateListForSession:(RimeSessionId)sessionId
{
    ...
    if (ite.candidate.text) {
        printf("----------------->text----is exist");
    }
